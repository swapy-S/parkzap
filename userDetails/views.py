...
def submit_details(request):
	...
	if request.method == "POST":
		...
		date_str = dob 
		format_str = '%Y%m%d'
		datetime_obj = datetime.datetime.strptime(date_str.replace('-',''), format_str)
		...
		if(s>datetime.timedelta(18*365) and len(number) == 10):
			...
			messages.error(request,"Age is less than 18 and/or number length should be equal to 10")
			return render(request,'home.html')
